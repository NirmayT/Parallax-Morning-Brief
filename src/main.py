...
def _recent_titles(reference_dt):
    try:
        ...
        return [line.strip().lstrip("- ").split(" (covered", 1)[0] for line in str(block).splitlines() if line.strip() and line.strip().lower() != "none."]
    except Exception:
        return []
...
def _redact(value):
    if isinstance(value, dict):
        return {key: _redact(item) for key, item in value.items() if key != "evidence"}
...
def run(dry_run=False, force=False):
    ...
    try:
        ...
        market = {"equities": {}, "fx": {}, "rates": {}, "data_ok": False}
    try:
    ...
    records = email_parser.parse_messages([m for m in messages if m.get("id") not in processed])
    newsletters = newsletter_filter.filter_and_categorize(records, reference_dt)
    package = story_ranker.build_ranked_package(newsletters, market, _recent_titles(reference_dt))
    try:
        path = os.path.join(config.DEBUG_DIR, f"ranking_{reference_dt:%Y%m%d_%H%M%S}.json")
        with open(path, "w", encoding="utf-8") as handle:
            json.dump(_redact(package), handle, indent=2, ensure_ascii=False, default=str)
        utils.log(f"[RANK] Audit saved: {path}")
    except Exception as exc:
        utils.log(f"[RANK] Audit save failed: {exc}")

    ai = synthesis.synthesize(market, package, reference_dt)
    rank_pass = bool(package.get("quality", {}).get("pass", True))
    synth_pass = bool(ai.get("quality", {}).get("pass"))
    publish_ready = bool(rank_pass and synth_pass and ai.get("top_headlines"))
    status = {
        "Relevant newsletters": newsletters.get("stats", {}).get("relevant_sources", 0),
        "Rank quality": rank_pass,
        "Synthesis quality": synth_pass,
        "Publish ready": publish_ready,
        "AI engine": ai.get("engine", "unknown"),
    }
    plain, html = brief_builder.build_brief(reference_dt, market, ai, status)
    if not publish_ready:
        warning = '<div style="max-width:600px;margin:0 auto 12px;padding:10px;background:#fff3cd;color:#664d03;font-family:Arial,sans-serif;font-size:12px;text-align:center;"><strong>INTERNAL REVIEW WARNING:</strong> This edition did not pass the automated quality gate. Do not publish without manual review.</div>'
        html = warning + html
...
def parse_args(argv):
    return "--dry-run" in argv, "--force" in argv
